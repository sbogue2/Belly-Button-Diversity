...
function buildCharts(sample) {
  ...
  d3.json("samples.json").then((data) => {
    ...
    var ylabels = otu_ids.slice(0, 10).reverse().map(entry => `OTU ${entry.toString()}`);
    var yhoverLabels = otu_labels.slice(0, 10).reverse();  
...
Plotly.newPlot("bar", barData, barLayout);
    ...
    var bubblePlotData = [bubblePlotTrace];
    ...
    var gaugePlotData = [gaugePlotTrace];
